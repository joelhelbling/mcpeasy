...
class MCPServer
  ...
  def run
    ...
    File.write("./logs/mcp_gmeet_startup.log", "#{Time.now}: Google Meet MCP Server starting on stdio\n", mode: "a")
    ...
    File.write("./logs/mcp_gmeet_error.log", "#{Time.now}: #{e.message}\n#{e.backtrace.join("\n")}\n", mode: "a")
  end
  ...
  def handle_request(line)
    ...
    begin
      ...
      File.write("./logs/mcp_gmeet_error.log", "#{Time.now}: Error handling request: #{e.message}\n#{e.backtrace.join("\n")}\n", mode: "a")
      error_response = {
        jsonrpc: "2.0",
        id: request&.dig("id"),
        error: {
  ...
  def tools_call_response(id, params)
    ...
    begin
      ...
      File.write("./logs/mcp_gmeet_error.log", "#{Time.now}: Tool error: #{e.message}\n#{e.backtrace.join("\n")}\n", mode: "a")
      {
        jsonrpc: "2.0",
        id: id,
        result: {
